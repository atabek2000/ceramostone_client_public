<template>
  <div class="user-table">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="order-col buttons">
            ...
            <div
              ...
              "
              :title="$t('modal.ready')"
              @click="showReadyModal(record.id)"
            >
              <svg
                width="24"
                height="24"
    ...
    <a-modal
      ...
      :title="$t('title.selectSpecialist')"
      @ok="readyConfirm"
    >
      <a-row :gutter="[16, 16]">
        <a-col :xs="24">
          <a-select
            :placeholder="$t('button.specialist')"
            :options="$store.state.users.filter((user) => user.roleId === 10)"
            :field-names="{ label: 'name', value: 'id' }"
            v-model:value="readyModal.specialistId"
            style="width: 100%"
          ></a-select>
        </a-col>
        <a-col :xs="24">
          <a-input
            v-model:value="readyModal.comment_spec"
            placeholder="Коммент"
          />
        </a-col>
      </a-row>
...
import columns from "@/utils/purchases/purchaseDirector1Cols";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const readyModal = reactive({
      ...
      orderId: undefined,
      comment_spec: undefined,
    });

    const showReadyModal = (orderId) => {
      readyModal.orderId = orderId;
      readyModal.visible = true;
    };
    const readyConfirm = () => {
      if (readyModal.specialistId) {
        emit("onReady", {
          orderId: readyModal.orderId,
          specialistId: readyModal.specialistId,
          comment_spec: readyModal.comment_spec,
          refreshTable: refresh,
        });
        readyModal.orderId = undefined;
        readyModal.specialistId = undefined;
        readyModal.comment_spec = undefined;
        readyModal.visible = false;
      } else {
        message.error(t("title.selectSpecialist"));
      }
    };
    const {
      data: dataSource,
